...
// struct. Zero value = nil root, no errors.
//
// root is an anonymous block — no key, no op — whose children are the file's
// top-level items: a file is a braceless block, and the whole read API
// (get/item/read_*) works on it directly. On errors the parser recovers and
// keeps going, so the tree holds whatever could still be parsed (possibly
// nothing).
struct ParseResult {
    Node              root;
...
fn ParseResult parse(arena::Arena* arena, String src) {
    ...
    result.root.children = parse_items(&p, 0);
